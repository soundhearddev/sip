...
fn printIdentityEntry(ctx: *ListCtx, entry: sip.IdentityEntry) !void {
    if (!entry.valid) {
    ...
    const base = sip.baseAddress(entry.public);
    var addr_buf: [80]u8 = undefined;
    const addr = try sip.formatSipAddress(&addr_buf, base);
    ...
    if (ctx.verbose) {
        ...
        const dpath = try sip.identityDir(&dir_buf, entry.name());
...
fn listIdentities(io: std.Io, stdout: *Io.Writer, verbose: bool) !void {
    ...
    sip.forEachIdentity(io, *ListCtx, &ctx, struct {
        fn cb(c: *ListCtx, entry: sip.IdentityEntry) !void {
            try printIdentityEntry(c, entry);
        }
    }.cb) catch |err| switch (err) {
        sip.ListError.KeyRootMissing => {
...
fn cmdNew(io: std.Io, stdout: *Io.Writer, env_map: *const std.process.Environ.Map, args: *ArgIter) !void {
    ...
    const kp = sip.createIdentity(io, name, password) catch |err| switch (err) {
        sip.SipError.IdentityAlreadyExists => {
    ...
    const base = sip.baseAddress(kp.public);
    var addr_buf: [80]u8 = undefined;
    const addr = try sip.formatSipAddress(&addr_buf, base);
...
fn cmdShow(io: std.Io, stdout: *Io.Writer, args: *ArgIter) !void {
    ...
    const pub_bytes = sip.loadPublicOnly(io, name) catch |err| switch (err) {
        sip.SipError.IdentityNotFound => {
    ...
    const base = sip.baseAddress(pub_bytes);
    var addr_buf: [80]u8 = undefined;
    const addr = try sip.formatSipAddress(&addr_buf, base);

    var dir_buf: [300]u8 = undefined;
    const dpath = try sip.identityDir(&dir_buf, name);
...
fn cmdId(io: std.Io, stdout: *Io.Writer, args: *ArgIter) !void {
    ...
    const id = sip.genId(io, pub_bytes);
    try stdout.print("{x}\n", .{id});
    try stdout.flush();
}
...
fn cmdRemove(io: std.Io, stdout: *Io.Writer, args: *ArgIter) !void {
    ...
    sip.deleteIdentity(io, name) catch |err| switch (err) {
        sip.SipError.IdentityNotFound => {
            try stdout.print("Fehler: Identität '{s}' nicht gefunden.\n", .{name});
            try stdout.flush();
            return;
...
fn cmdPasswd(io: std.Io, stdout: *Io.Writer, env_map: *const std.process.Environ.Map, args: *ArgIter) !void {
    ...
    _ = sip.changePassword(io, name, old_pw, new_pw) catch |err| switch (err) {
        sip.SipError.IdentityNotFound => {
            try stdout.print("Fehler: Identität '{s}' nicht gefunden.\n", .{name});
            try stdout.flush();
            return;
        },
        sip.SipError.DecryptionFailed => {
