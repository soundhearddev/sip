...
pub fn build(b: *std.Build) void {
    ...
    _ = b.addModule("protocol", .{ .root_source_file = b.path("src/protocol.zig") });
    _ = b.addModule("synet", .{ .root_source_file = b.path("src/synet.zig") });
    _ = b.addModule("sip", .{ .root_source_file = b.path("src/sip.zig") });

    const pub_header_mod = b.addModule("header", .{ .root_source_file = b.path("src/header.zig") });
    pub_header_mod.addImport("protocol", protocol_mod);

    const pub_translation_mod = b.addModule("translation", .{ .root_source_file = b.path("src/translation.zig") });
    pub_translation_mod.addImport("header", header_mod);
    pub_translation_mod.addImport("protocol", protocol_mod);
    pub_translation_mod.addImport("synet", synet_mod);
